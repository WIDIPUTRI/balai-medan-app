...
@section('title', 'Update Realisasi')
@section('page-title', 'Update Realisasi Anggaran')
...
<div class="p-6">
    <div class="bg-gradient-to-br from-white to-cyan-50 shadow-lg rounded-xl p-6 border border-cyan-100">
        ...
        <div class="space-y-6">
            @forelse($groupedAnggaran as $programName => $akuns)
            <div class="border rounded-xl overflow-hidden border-cyan-200 shadow-sm bg-white" x-data="{ open: false }">
                ...
                <button @click="open = !open" 
                    ...
                    <div class="flex items-center gap-3">
                        <span class="font-bold text-cyan-900 text-sm md:text-base">{{ $programName }}</span>
                    ...
                    <table class="min-w-full text-sm">
                        ...
                        <tbody class="divide-y divide-sky-50">
                            ...
                            <tr class="hover:bg-cyan-50/60 transition relative group">
                                <td class="p-4 font-mono text-cyan-600 font-medium">{{ $akun->kode }}</td>
                                <td class="p-4">
                                    <div class="font-bold text-gray-800">{{ $akun->uraian }}</div>
                                    <div class="text-xs text-cyan-600 mt-1 flex items-center gap-1">
                                        <i class="fa-solid fa-turn-up rotate-90 text-[10px] opacity-50"></i>
                                        <span>{{ $akun->parent->uraian }}</span>
                                    </div>
                                </td>
                                <td class="p-4 text-right whitespace-nowrap text-gray-600">{{ number_format($akun->pagu_revisi, 0, ',', '.') }}</td>
                                <td class="p-4 text-right whitespace-nowrap font-bold text-blue-600 bg-blue-50/50 rounded-l-lg my-1">{{ number_format($akun->realisasi_ini, 0, ',', '.') }}</td>
                                <td class="p-4 text-right whitespace-nowrap {{ $akun->sisa_anggaran < 0 ? 'text-red-600 font-bold' : 'text-emerald-600 font-semibold' }}">
                                    {{ number_format($akun->sisa_anggaran, 0, ',', '.') }}
                                </td>
                                <td class="p-4 text-center">
                                    <button type="button" onclick="event.stopPropagation(); openModal('{{ $akun->id }}', '{{ $akun->uraian }}')" 
                            ...
                            @if($akun->realisasiDetails->count() > 0)
                            <tr class="bg-amber-50/50 hidden group-hover:table-row transition-all border-l-4 border-amber-300">
                                <td colspan="6" class="px-12 py-3 text-xs text-amber-800">
                                    ...
                                    {{ $akun->realisasiDetails->first()->tanggal->format('d M Y') }} 
                                    — <span class="font-mono font-bold">Rp {{ number_format($akun->realisasiDetails->first()->jumlah, 0, ',', '.') }}</span>
                                </td>
                            </tr>
                            @endif
                            @endforeach
        ...
        <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            <div class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-lg border border-white/20">
                ...
                <div class="bg-gradient-to-br from-white to-cyan-50 px-6 py-6">
                    ...
                    <form id="realisasiForm" action="{{ route('anggaran.storeRealisasi') }}" method="POST" class="space-y-5">
                        @csrf
