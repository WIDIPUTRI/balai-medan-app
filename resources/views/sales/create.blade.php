                ...
                <div class="bg-white rounded-lg border border-gray-200 p-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 max-h-96 overflow-y-auto">
                        ...
                        <div x-show="'{{ strtolower($product->name) }} {{ strtolower($product->code) }}'.includes(searchProduct.toLowerCase())" 
                            @click="addItem({{ $product->id }}, '{{ $product->code }}', '{{ $product->name }}', {{ $product->selling_price }}, {{ $product->stock }})"
                            ...
                            <div class="flex items-center space-x-3">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <p class="text-sm font-medium text-primary">{{ formatRupiah($product->selling_price) }}</p>
                ...
                <div class="bg-white rounded-lg border border-gray-200 p-6 sticky top-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="space-y-2">
                            ...
                            <a href="{{ route('sales.index') }}" class="block w-full px-4 py-3 border border-gray-300 rounded-lg text-center text-gray-700 hover:bg-gray-50 transition duration-200">
                                Batal
                            </a>
                        </div>
                    </div>
...
function saleForm() {
    return {
        ...
        addItem(id, code, name, price, maxStock) {
            const existingItem = this.items.find(item => item.id === id);
            if (existingItem) {
                if (existingItem.quantity < maxStock) {
                    existingItem.quantity++;
                } else {
                    showAlert('error', 'Stok tidak tersedia');
                }
            } else {
                this.items.push({ id, code, name, price, quantity: 1, maxStock });
            }
        },
        
        increaseQty(index) {
            if (this.items[index].quantity < this.items[index].maxStock) {
