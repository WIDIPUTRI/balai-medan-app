    ...
    <div class="bg-white rounded-lg border border-gray-200 p-6 mb-6">
        <div class="mb-6 pb-6 border-b border-gray-200">
            <div class="flex justify-between items-start">
                <div>
                    <h2 class="text-2xl font-bold text-gray-900">Detail Penjualan</h2>
                    <p class="text-sm text-gray-600 mt-1">{{ $sale->sale_date->isoFormat('dddd, D MMMM Y') }}</p>
                </div>
                <div class="text-right">
                    <p class="text-sm text-gray-600">Nama Toko</p>
                    <p class="text-lg font-semibold text-gray-900">{{ $sale->store->name }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="space-y-2 mb-6 pb-6 border-b border-gray-200">
            <div class="flex justify-between text-lg font-bold pt-2">
                ...
                <span class="text-primary">{{ formatRupiah($sale->total) }}</span>
            </div>
        </div>
        ...
        <div class="p-4 bg-blue-50 border border-blue-200 rounded-lg mb-6">
            <h3 class="text-sm font-medium text-gray-900 mb-2">Informasi Pengiriman</h3>
            <div class="grid grid-cols-2 gap-4 text-sm">
                <div>
                    <p class="text-gray-600">Kategori</p>
                    <p class="font-medium text-gray-900">{{ ucfirst($sale->shipment->category) }}</p>
                </div>
                <div>
                    <p class="text-gray-600">Biaya Pengiriman</p>
                    <p class="font-medium text-gray-900">{{ formatRupiah($sale->shipment->shipping_cost) }}</p>
                </div>
            </div>
        </div>
        @endif
        ...
        <div class="flex justify-end space-x-4 pt-6 border-t border-gray-200">
            <a href="{{ route('sales.index') }}" class="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition duration-200">
                Kembali
            </a>
