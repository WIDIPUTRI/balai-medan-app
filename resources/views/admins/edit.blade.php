        ...
        <form action="{{ route('admins.update', $admin) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="birth_place" id="birth_place" value="{{ old('birth_place', $admin->birth_place) }}"
                ...
                <div>
                    ...
                    <input type="date" name="birth_date" id="birth_date" value="{{ old('birth_date', $admin->birth_date?->format('Y-m-d')) }}"
                ...
                <div>
                    ...
                    <input type="text" name="education" id="education" value="{{ old('education', $admin->education) }}"
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="address" id="address" rows="3"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary @error('address') border-red-500 @enderror">{{ old('address', $admin->address) }}</textarea>
                    @error('address')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label for="password" class="block text-sm font-medium text-gray-700 mb-2">Password <span class="text-xs text-gray-500">(kosongkan jika tidak diubah)</span></label>
                ...
                <div class="md:col-span-2">
                    ...
                    @if($admin->photo)
                    <div class="mb-2">
                        <img src="{{ asset('storage/' . $admin->photo) }}" alt="{{ $admin->name }}" class="w-32 h-32 object-cover rounded">
                    </div>
                    @endif
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="notes" id="notes" rows="3"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary @error('notes') border-red-500 @enderror">{{ old('notes', $admin->notes) }}</textarea>
                    @error('notes')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="flex justify-end space-x-4 mt-6 pt-6 border-t border-gray-200">
                <a href="{{ route('admins.index') }}" class="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition duration-200">
                    Batal
                </a>
